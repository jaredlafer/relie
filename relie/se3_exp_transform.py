...
class RestrictedSE3ExpTransform(Transform):
...
class SE3ExpCompactTransform(LocalDiffeoTransform):
    """Assumes underlying rotation distribution has support only in the <2pi ball."""
    ...
    def __init__(self, support_radius=2 * math.pi):
        """
        :param support_radius: radius of inverse set (for rotation part only)
        """
        super().__init__()
